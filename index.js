...
function createDictionary(entries) {
  ...
  entries.forEach((arr) => map.set(arr[0], arr[1])); // Використання методу forEach для перебору масиву пар ключ-значення // Додавання пари ключ-значення до словника за допомогою методу set
  return map;
}
...
function convertMapToObject(map) {
  // const iter = map.entries(); // Перебираємо ключі та значення в словнику
  const obj = Object.fromEntries(map); // Додаємо ключ та значення до об'єкту
...
function setValue(dictionary, key, value) {
  if (dictionary.has(key)) {
    ...
    dictionary.set(key, value.toUpperCase()); // Якщо ключ вже існує, встановлюємо для нього нове значення в верхньому регістрі
  } else {
...
function getKeysStartingWith(dictionary, letter) {
  const arr = []; // Оголошення порожнього масиву для збереження ключів
...
function getFilteredDictionarySize(dictionary, filter) {
  const fltr = new Map(); // Створюємо новий Map об'єкт для зберігання елементів, що відповідають фільтру
  ...
  return fltr.size; // Повертаємо розмір фільтрованого словника, використовуючи властивість size
}
...
function sortByValues(dictionary) {
  ...
  const sortedEnries = [...entries].sort((a, b) => b[1] - a[1]);
  // Сортуємо масив пар ключ-значення за значеннями в порядку спадання
  const newDict = new Map(sortedEnries); // Конвертуємо відсортований масив пар ключ-значення назад у словник
  return newDict;
}
...
function resetDictionary(dictionary, maxSize) {
  let amount = dictionary.size; // Використовуємо метод size для перевірки кількості елементів у словнику
  if (amount > maxSize) {
    dictionary.clear();
    return true;
  ...
  return false; // Повертаємо false, оскільки кількість елементів не перевищує максимально допустиму, тому словник не був очищений
}
...
function convertDictionaryToSet(dictionary) {
  let result = new Set(); // Перебираємо ключі словника
  for (const key of dictionary.keys()) {
    if (typeof key === "string") {
      // Якщо ключ є рядком, переводимо його в верхній регістр і додаємо до множини
      result.add(key.toUpperCase());
    } else {
      result.add(key + 1); // Якщо ключ є числом, збільшуємо його на 1 і додаємо до множини
...
function convertSetToDictionary(set) {
  ...
  for (const value of set) {
    result.set(value, value.charCodeAt(0));
  }
  ...
  return result; // Повертаємо отриманий словник
}
